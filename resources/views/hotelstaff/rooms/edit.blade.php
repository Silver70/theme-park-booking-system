    ...
    <x-sidebar :menuItems="$menuItems" title="Edit Room">
        <div class="space-y-6">
            ...
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Edit Room</h1>
                    <p class="text-gray-600 dark:text-gray-400">Update room information.</p>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
                <form method="POST" action="{{ route('hotelstaff.rooms.update', $room->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <!-- Room Image -->
                        <div>
                            <label for="image" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Room Image (Optional)</label>
                            
                            <!-- Current Image Preview -->
                            @if($room->image)
                                <div class="mt-2 mb-4">
                                    <p class="text-sm text-gray-600 dark:text-gray-400 mb-2">Current Image:</p>
                                    <img src="{{ asset('storage/' . $room->image) }}" alt="Current room image" class="h-32 w-48 object-cover rounded-lg border border-gray-300 dark:border-gray-600">
                                </div>
                            @endif
                                ...
                                <div class="space-y-1 text-center">
                                    ...
                                    <div class="flex text-sm text-gray-600 dark:text-gray-400">
                                        <label for="image" class="relative cursor-pointer bg-white dark:bg-gray-800 rounded-md font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
                                            <span>{{ $room->image ? 'Replace image' : 'Upload a file' }}</span>
